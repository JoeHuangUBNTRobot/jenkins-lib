 	...
 	echo "build $productSeries"
   	...
   	build_jobs.add([
			...
			pre_checkout_steps: { m->
				...
				m.upload_info = [path: ["${m.name}", "arm64", "${m.build_dir}"], latest_path: ["${m.name}", "arm64", "latest"]]
			},
			build_steps: { m ->
			    ...
			    ws("${m.build_dir}") {
			        ...
			        sh "./preload_image $bootload_path $unvr4_fcd_uImage $unvr4_preload $unvrpro_fcd_uImage $unvrpro_preload $unvrai_fcd_uImage $unvrai_preload"
